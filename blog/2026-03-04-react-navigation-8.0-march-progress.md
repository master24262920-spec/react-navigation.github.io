...
We've been busy since the [first alpha release of React Navigation 8.0](/blog/2025/12/19/react-navigation-8.0-alpha), and we wanted to share what's been happening.
...
We have added a new `inactiveBehavior` option to all navigators that gives you control over how inactive screens are handled. Currently it supports 2 values:
...
Any subscriptions, timers etc. are cleaned up for paused screens, which would avoid re-rendering inactive screens unnecessarily.
...
We also plan to add a `unmount` option for the [Native Stack](/docs/8.x/native-stack-navigator) and [JS Stack](/docs/8.x/stack-navigator) in the future that would unmount inactive screens entirely.
...
<img src="/assets/themes/material-light-1.png" style={{ width: '360px' }} />
<img src="/assets/themes/material-light-2.png" style={{ width: '360px' }} />
<img src="/assets/themes/material-dark-1.png" style={{ width: '360px' }} />
<img src="/assets/themes/material-dark-2.png" style={{ width: '360px' }} />
...
### `getId` behavior change in stack navigators
...
- A list of all documentation pages and full documentation content is available at [`llms.txt`](/llms.txt) and [`llms-full.txt`](/llms-full.txt) respectively

Hopefully these will make it easier to use React Navigation with LLMs.

## Plans for the future

There are a few more plans we can share now that we have more clarity on the roadmap:

- **React Native Screens 5 integration** - The [`react-native-screens`](https://github.com/software-mansion/react-native-screens) library is being rewritten with a new implementation for Native Stack. We plan to use it for `@react-navigation/native-stack` and `@react-navigation/bottom-tabs` and will try our best to keep the same API as much as possible.
- **Ability to unmount inactive screens** - We plan to add support for `inactiveBehavior: 'unmount'` to unmount inactive screens entirely in stack navigators, which would be useful if you can have a large number of screens in the stack.
- **Data loading** - We plan to add a `loader` prop to screen configuration in static config that implements "render-as-you-fetch" pattern for data loading that works with [React Suspense](https://react.dev/reference/react/Suspense). Our current goal is to provide a way to integrate with existing libraries such as React Query so you can use their battle-tested data fetching and caching logic.
